...
import { generateAndSendVerificationEmail } from "../../utils/verificationHelper.js";
...
export const signup = async (req, res) => {
  try {
    ...
    // const verifyUrl = `http://localhost:5173/verify-mail/`;
    // await sendEmail(verifyUrl, email);

    const { password: _, ...userWithoutPassword } = user.toObject();
    res
      .status(201)
      .json({ message: "Signup successful", user: userWithoutPassword });
  } catch (error) {
